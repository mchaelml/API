...
exports.add = item => {
	...
	if (storage.getItem(item) === undefined) {
		...
		storage.setItem(item, {index : storage.length,title: item, qty: 1})
	} else {
		...
		storage.setItem(item, {index :storage.length,title: item, qty: current+1})
	}
	return true
};
